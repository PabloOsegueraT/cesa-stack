...
// ========================
//  Dashboard PERSONAL (solo mis tareas)
// ========================
// GET /api/dashboard/my-summary?year=2025&month=11
api.get('/dashboard/my-summary', requireAnyAuthenticated, async (req, res) => {
  try {
    const userId = req.authUserId;
    ...
    // 1) Conteo por estado, SOLO tareas asignadas a este usuario
    const [statusRows] = await pool.query(
      `
        SELECT t.current_status_id AS status_id, COUNT(*) AS count
        FROM tasks t
    ...
    // 2) Conteo por prioridad, SOLO mis tareas
    const [priorityRows] = await pool.query(
      `
        SELECT t.priority_id AS priority_id, COUNT(*) AS count
        FROM tasks t
    ...
    // 3) Mis tareas que vencen en las próximas 48 horas
    const [dueSoonRows] = await pool.query(
      `
        SELECT COUNT(*) AS c
        FROM tasks t
        ...
        WHERE t.archived = 0
          ...
          AND t.current_status_id IN (?, ?)
      `,
      [userId, STATUS_IDS.pending, STATUS_IDS.in_progress]
    );
    ...
    console.error('Error en /dashboard/my-summary:', err);
    return res
      .status(500)
      .json({ message: 'Error al obtener resumen personal' });
  }
});
